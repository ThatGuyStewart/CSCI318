...
|**C1 / C2**|`GET`|`/customer/id/{id}`|*None*|`200 OK` (`CustomerResponse`)|*None*|Get customer details by id|
...
|Feature|Method|Endpoint Path|Request Body|Response (Success)|Inter-Service REST Dependencies|Description|
|-|-|-|-|-|-|-|
|**O1**|`POST`|`/order`|`OrderCreateRequest`|`201 Created` (`OrderResponse`)|**Customer Service:** `GET /customer/id/{id}`; **Product Service:** `GET /product/{id}`; **Notification Service:** `POST /notification`|Create an order (from basket or explicit product/quantity)|
...
|**O2**|`GET`|`/order/customer/id/{customerId}`|*None*|`200 OK` (`Array<OrderResponse>`)|*None*|View all orders by customer id|
...
|**O2**|`GET`|`/order/customer/id/{customerId}/event`|Optional Query params `date`, or `from` and `to`|`200 OK` (`Array<OrderDomainEventResponse>`)|*None*|View all order domain events for orders by customer id, with option to filter by date or date range (inclusive)|
...
|Feature|Method|Endpoint Path|Request Body|Response (Success)|Inter-Service REST Dependencies|Description|
|-|-|-|-|-|-|-|
|**N1**|`POST`|`/notification/customer/id/{customerId}`|`NotificationCreateRequest`|`201 Created` (`NotificationResponse`)|**Customer Service:** `GET /customer/id/{id}`|Create notification by customer id|
...
|**N1**|`GET`|`/notification/customer/id/{customerId}`|Optional Query params `date`, or `from` and `to`|`200 OK` (`Array<NotificationResponse>`)|*None*|View notifications by customer id, with option to filter by date or date range (inclusive)|
