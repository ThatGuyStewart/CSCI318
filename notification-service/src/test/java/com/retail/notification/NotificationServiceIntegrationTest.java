...
class NotificationServiceIntegrationTest {
        ...
        void setUp() {
                ...
                AddressDto addr2 = new AddressDto(null, 20, "Bourke St", "Melbourne", "Melbourne", 3000, "VIC",
                                "Australia");
        ...
        void testCreateAndGetByCustomerId_HappyPath() throws Exception {
                ...
                // Get by ID
                mockMvc.perform(get("/notification/" + created.getNotificationId()))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$.notificationId").value(created.getNotificationId()));
                ...
                org.junit.jupiter.api.Assertions.assertTrue(notificationViewRepository.existsById(created.getNotificationId()));
        ...
        void testBroadcastNotifications() throws Exception {
                NotificationBroadcastRequest broadcast = new NotificationBroadcastRequest(
                                "Storewide discount this weekend!");
        ...
        void notificationCommandsAppendImmutablePersistedEvents() throws Exception {
                ...
                mockMvc.perform(post("/notification/customer/1")
                                ...
                                .content(objectMapper
                                                .writeValueAsString(new NotificationCreateRequest("Original message"))))
                                .andExpect(status().isCreated());
                mockMvc.perform(post("/notification/customer/email/alice@test.com")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper
                                                .writeValueAsString(new NotificationCreateRequest("Email message"))))
                                .andExpect(status().isCreated());
                mockMvc.perform(post("/notification/customer/phone/0400333444")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper
                                                .writeValueAsString(new NotificationCreateRequest("Phone message"))))
                                .andExpect(status().isCreated());
                mockMvc.perform(post("/notification/broadcast")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(
                                                new NotificationBroadcastRequest("Broadcast message"))))
                                .andExpect(status().isCreated());
                mockMvc.perform(post("/notification/broadcast/area")
                                .contentType(MediaType.APPLICATION_JSON)
                                .content(objectMapper.writeValueAsString(
                                                new NotificationAreaBroadcastRequest("Area message", null, "NSW",
                                                                null))))
                                .andExpect(status().isCreated());
                ...
                org.junit.jupiter.api.Assertions.assertTrue(payload.has("sentAt"));
                org.junit.jupiter.api.Assertions.assertFalse(payload.has("sent"));
                Notification notification = notificationRepository.findById(payload.get("notificationId").asLong())
                                .orElseThrow();
                ...
                org.junit.jupiter.api.Assertions.assertEquals("Original message", objectMapper.readTree(
                                notificationDomainEventRepository.findById(first.getEventId()).orElseThrow()
                                                .getPayload())
                                .get("message").asText());
        }
        ...
        void testDateRangeQueries() throws Exception {
                ...
                // Collection endpoints support the equivalent optional contract filters.
                mockMvc.perform(get("/notification").param("date", today.toString()))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$", hasSize(greaterThanOrEqualTo(1))));

                mockMvc.perform(get("/notification/customer/1").param("from", today.minusDays(1).toString())
                                .param("to", today.plusDays(1).toString()))
                                .andExpect(status().isOk())
                                .andExpect(jsonPath("$", hasSize(greaterThanOrEqualTo(1))));
        }
        ...
        @Test
        void notificationDateFiltersRejectInvalidCombinations() throws Exception {
                LocalDate today = LocalDate.now();
                ...
                mockMvc.perform(get("/notification/customer/1").param("from", today.toString()))
                                .andExpect(status().isBadRequest())
                                .andExpect(jsonPath("$.code").value("BAD_REQUEST"));

                mockMvc.perform(get("/notification/customer/email/alice@test.com").param("to", today.toString()))
                                .andExpect(status().isBadRequest())
                                .andExpect(jsonPath("$.code").value("BAD_REQUEST"));

                mockMvc.perform(get("/notification/customer/phone/0400333444")
                                .param("from", today.plusDays(1).toString())
                                .param("to", today.toString()))
                                .andExpect(status().isBadRequest())
                                .andExpect(jsonPath("$.code").value("BAD_REQUEST"));

                mockMvc.perform(get("/notification/date-range").param("from", today.plusDays(1).toString())
                                .param("to", today.toString()))
                                .andExpect(status().isBadRequest())
                                .andExpect(jsonPath("$.code").value("BAD_REQUEST"));
        }
